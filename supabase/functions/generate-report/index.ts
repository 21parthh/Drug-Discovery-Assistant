...
serve(async (req) => {
  ...
  try {
    ...
    if (!discoveryResult || !discoveryResult.targets) {
      return new Response(
        JSON.stringify({ error: 'Invalid discovery result data' }),
...
Total compounds analyzed: ${discoveryResult.targets.reduce((sum: number, target: any) => sum + target.hits.length, 0)}

Target Details:
${discoveryResult.targets.map((target: any, index: number) => `
${index + 1}. Target: ${target.symbol}
   ...
   ${target.hits.slice(0, 3).map((hit: any, hitIndex: number) => `
     Compound ${hitIndex + 1}:
...
`;
    ...
    return new Response(JSON.stringify({ 
      ...
      metadata: {
        ...
        compounds_count: discoveryResult.targets.reduce((sum: number, target: any) => sum + target.hits.length, 0),
